...
- `.github/workflows/deploy.yml`：GitHub Actionsで1時間ごとに自動ビルド・デプロイ（add_blog_posts → build → gh-pages）
...
### ヘルパー関数
- `getText(page, key)`：Notionのrich_text/titleを全セグメント結合して返す（ハイパーリンクを含む場合も正しく取得）
