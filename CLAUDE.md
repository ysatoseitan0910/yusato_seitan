...
- **サブセット取り寄せ**：入力欄は Zen Maru Gothic なので、別フォントを選ぶと描く文字のサブセットが
  未取得のまま別フォントに落ちる。 がフォントごとに描いた文字を集めて
   し、完了後に描き直す。保存は  を待ってから
- **下書き（localStorage）は **。復元値は  で許可リストに丸める。
  v1 には修正前の  入りフォント指定が残っていて、復元すると本文が描けなかった
- **送信は確認ダイアログを1段はさむ**。送信後も画面の内容は消さない（PNG保存し忘れ対策）。
  下書きだけ消して「送信済み」表示にし、内容を変えるまで再送信できない
- **PNG保存は iOS で Web Share にフォールバック**（ が効かないため。card.html と同じ）
- **サーバ側**： は （UTC だと JST 0〜9時が前日になる）、レート制限は 10件/時/IP
  （3件だと同じWi-Fiの3人目が弾かれる）
- **OGP画像は **（カードの見本入り・1200×630）。差し替えるときはファイル名を変える
